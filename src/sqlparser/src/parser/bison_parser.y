...
%union {
  ...
  hsql::BinlogOption* binlog_option;
  ...
  std::vector<hsql::BinlogOption*>* binlog_option_vec;
  ...
  hsql::RowLockWaitPolicy lock_wait_policy_t;
    ...
    %type <tenant_name>            tenant_name opt_for
...
| SHOW SESSION VARIABLES LIKE expr {
  $$ = new ShowStatement(kShowVar);
  $$->_var_name = $5->name;
  $$->var_type = Session;
}
...
| SHOW LOCAL VARIABLES LIKE expr {
  $$ = new ShowStatement(kShowVar);
  $$->_var_name = $5->name;
  $$->var_type = Local;
}
...
| SHOW GLOBAL VARIABLES LIKE expr {
  $$ = new ShowStatement(kShowVar);
  $$->_var_name = $5->name;
}
...
show_binlog_status_statement: SHOW BINLOG STATUS opt_for {
  $$ = new ShowBinlogStatusStatement();
  $$->tenant=$4;
};

show_binlog_server_statement: SHOW BINLOG SERVER opt_for {
  $$ = new ShowBinlogServerStatement();
  $$->tenant=$4;
};


purge_binlog_statement: PURGE BINARY LOGS TO expr opt_for{
  $$ = new PurgeBinlogStatement();
  $$->binlog_file=$5;
  $$->tenant=$6;
}
| PURGE BINARY LOGS BEFORE expr opt_for{
  $$ = new PurgeBinlogStatement();
  $$->purge_ts=$5;
  $$->tenant=$6;
};
...
create_binlog_statement:CREATE BINLOG opt_not_exists opt_for opt_user_info opt_from binlog_option_clause {
...
drop_binlog_statement:DROP BINLOG opt_exists opt_for {
...
| LIMIT expr ',' expr { $$ = new LimitDescription($2, $4); }
...
opt_for : FOR TENANT IDENTIFIER '.' IDENTIFIER {$$ = new TenantName($3, $5);}
| /* empty */ { $$ = nullptr; };
...
binlog_option: SERVER UUID IDENTIFIER {
  $$ = new BinlogOption(BinlogOptionType::SERVER_UUID,$3);
}
| INITIAL_TRX_GTID_SEQ IDENTIFIER {
  $$ = new BinlogOption(BinlogOptionType::INITIAL_TRX_GTID_SEQ,$2);
}
| INITIAL_TRX_XID IDENTIFIER {
  $$ = new BinlogOption(BinlogOptionType::INITIAL_TRX_XID,$2);
}
| INITIAL_TRX_SEEKING_ABORT_TIMESTAMP IDENTIFIER {
  $$ = new BinlogOption(BinlogOptionType::INITIAL_TRX_SEEKING_ABORT_TIMESTAMP,$2);
}
| SERVER_UUID IDENTIFIER {
  $$ = new BinlogOption(BinlogOptionType::SERVER_UUID,$2);
}
| CLUSTER_URL IDENTIFIER {
  $$ = new BinlogOption(BinlogOptionType::CLUSTER_URL,$2);
}
| CLUSTER URL IDENTIFIER {
  $$ = new BinlogOption(BinlogOptionType::CLUSTER_URL,$3);
}
;
