...
void printSetStatementInfo(const SetStatement* stmt, uintmax_t numIndent)
{
  inprint("SetStatement", numIndent);
  for (SetClause* set_clause : *stmt->sets) {
    inprint(set_clause->column, numIndent + 1);
    inprint(set_clause->value->ival, numIndent + 1);
  }
}
...
void printStatementInfo(const SQLStatement* stmt)
{
  switch (stmt->type()) {
    ...
    case COM_TRANSACTION:
      ...
      break;
    case COM_SET:
      printSetStatementInfo((const SetStatement*)stmt, 0);
      break;
    default:
      break;
  }
...
std::ostream& operator<<(std::ostream& os, const OperatorType& op)
{
  static const std::map<const OperatorType, const std::string> operatorToToken = {{kOpNone, "None"},
      {kOpBetween, "BETWEEN"},
      {kOpCase, "CASE"},
